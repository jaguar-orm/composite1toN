...
abstract class _AuthorBean implements Bean<Author> {
  ...
  Future<int> remove(String id, [bool cascade = false]) async {
    if (cascade) {
      ...
      await postBean.removeByAuthor(newModel.id, newModel.id1);
    }
  ...
  Future preload(Author model, {bool cascade: false}) async {
    model.posts = await postBean.findByAuthor(model.id, model.id1,
  ...
  Future preloadAll(List<Author> models, {bool cascade: false}) async {
    ...
    await PreloadHelper.preload<Author, Post>(
        models,
        (Author model) => [model.id, model.id1],
        postBean.findByAuthorList,
        (Post model) => [model.authorid, model.authorid1],
...
abstract class _PostBean implements Bean<Post> {
  ...
  final IntField authorid1 = new IntField('authorid1');
  ...
  List<SetColumn> toSetColumns(Post model, [bool update = false]) {
    ...
    ret.add(authorid1.set(model.authorid1));
  ...
  Future<List<Post>> findByAuthor(String authorid, int authorid1,
      {bool preload: false, bool cascade: false}) async {
    final Find find = finder
        .where(this.authorid.eq(authorid))
        .where(this.authorid1.eq(authorid1));
    return await (await execFind(find)).toList();
  }

  Future<int> removeByAuthor(String authorid, int authorid1) async {
  ...
  Future<List<Post>> findByAuthorList(List<Author> models,
    ...
    for (Author model in models) {
      find.or(this.authorid.eq(model.id) & this.authorid1.eq(model.id1));
    }
    return await (await execFind(find)).toList();
  }
